...
const MapPage = () => {
  ...
  const getAllCemeteryData = async () => {
    try {
      ...
      console.log('✅ Retrieved plots:', plots.length);
      return plots;
    } catch (error) {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        console.log('🔄 Starting to load cemetery data...');
        const allPlots = await getAllCemeteryData();
        console.log('✅ Cemetery data loaded:', allPlots.length, 'plots');
        setDataLoaded(true);
        ...
        console.error('❌ Error loading cemetery data:', error);
        console.log('🔄 Setting dataLoaded to true to prevent infinite loading');
        setDataLoaded(true);
      }
    };
    ...
    const immediateTimeoutId = setTimeout(() => {
      console.log('⚡ Immediate timeout - forcing map to load');
      setDataLoaded(true);
    }, 1000); // 1 second immediate timeout

    // Add 3-second timeout as backup
    const timeoutId = setTimeout(() => {
      console.log('⏰ 3-second timeout reached, forcing map to load');
      setDataLoaded(true);
